...
if __name__ == '__main__':
    # Example CDAHeaderData
    header = CDAHeaderData(
        document_id=II_Element(root="1.2.3.4", extension="doc1"),
        document_title="Health Checkup Report",
        patient_name_family="Yamada",
        patient_name_given="Taro"
    )
    if header.patient_gender:
        header.patient_gender.code = "1"
        header.patient_gender.code_system = "1.2.392.200119.6.1104"
        header.patient_gender.display_name = "Male"
    print(header)

    # Example ObservationDataItem
    height_obs = ObservationDataItem(
        item_code=CD_Element(code="PHY001", display_name="Height"),
        value="170",
        value_type="PQ",
        unit="cm"
    )
    ...
    checkup_record = HealthCheckupRecord(header=header)
    checkup_record.raw_input_data = {"csv_col1": "val1"}
    print(checkup_record)
    if checkup_record.header.document_id:
        print(f"Checkup Record Doc ID Ext: {checkup_record.header.document_id.extension}")

    print("-" * 20)
    # Example ObservationGroup
    blood_panel = ObservationGroup(
        panel_code=CD_Element(code="PNL001", display_name="Blood Panel")
    )
    blood_panel.components.append(ObservationDataItem(item_code=CD_Element(code="GLU001"), value="100", value_type="PQ", unit="mg/dL"))
    blood_panel.components.append(ObservationDataItem(item_code=CD_Element(code="CHOL01"), value="200", value_type="PQ", unit="mg/dL"))
    print(blood_panel)
    for comp in blood_panel.components:
        print(f"  Component: {comp.item_code.code if comp.item_code else 'N/A'}, Value: {comp.value}")

    print("-" * 20)
    # Example HealthGuidanceRecord
    guidance_header = CDAHeaderData(document_title="Health Guidance Plan")
    guidance_record = HealthGuidanceRecord(header=guidance_header)
    guidance_record.guidance_classification = ObservationDataItem(
        item_code=CD_Element(code="HGCLASS01", display_name="積極的支援")
    )
    print(guidance_record)
    if guidance_record.guidance_classification and guidance_record.guidance_classification.item_code:
            print(f"Guidance Class: {guidance_record.guidance_classification.item_code.display_name}")
    ...
    print("-" * 20)
    cs_record = CheckupSettlementRecord(document_id_ext="CS_DOC_001")
    cs_record.total_cost_value = "5000"
    print(cs_record)

    print("-" * 20)
    gs_record = GuidanceSettlementRecord()
    gs_record.document_id = II_Element(root="1.2.3", extension="GS_DOC_001")
    gs_record.total_cost_settlement = MO_Element_Data(value="300", currency="JPY") # Ensure MO_Element_Data is used
    print(gs_record)
    if gs_record.total_cost_settlement:
            print(f"GS Record Cost: {gs_record.total_cost_settlement.value} {gs_record.total_cost_settlement.currency}")
